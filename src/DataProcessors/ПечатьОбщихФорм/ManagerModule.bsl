...
Процедура СК_ЗаполнитьРеквизитыСтрокиТовара(СтрокаТовары, ОбластьМакета, НомерСтроки, ВыводитьКодНоменклатуры = Истина, ВыводитьКодТНВД = Неопределено, ВыводитьСерии = Ложь, СчетФактураНаАванс = Ложь)
	...
	Если СтрокаТовары.Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Услуга Тогда
		Попытка
			Если СтрДлина(СтрокаТовары.Ссылка.ДокументОснование.Товары[СтрокаТовары.НомерСтроки-1].Содержание) >=5 Тогда
				ПредставлениеНоменклатуры = СтрокаТовары.Ссылка.ДокументОснование.Товары[СтрокаТовары.НомерСтроки-1].Содержание;
			КонецЕсли;
		Исключение
