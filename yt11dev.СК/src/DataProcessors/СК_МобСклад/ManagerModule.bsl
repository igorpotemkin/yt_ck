...
Функция ПолучитьОстатокНоменклатураЯчейкаНаДату(Номенклатура,Ячейка,ДатаЗапроса) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	СК_ТоварыНаСкладахОстатки.Номенклатура КАК Номенклатура,
	               |	СК_ТоварыНаСкладахОстатки.Ячейка КАК Ячейка,
	               |	СК_ТоварыНаСкладахОстатки.ВналичииОстаток КАК ВналичииОстаток
	               |ИЗ
	               |	РегистрНакопления.СК_ТоварыНаСкладах.Остатки(&ДатаЗапроса,Номенклатура = &Номенклатура И Ячейка = &Ячейка) КАК СК_ТоварыНаСкладахОстатки
	               |
	               |	";
	Запрос.УстановитьПараметр("Номенклатура",Номенклатура); 
	Запрос.УстановитьПараметр("Ячейка",Ячейка); 
...
Функция ТабДокументовПересчет(СК_НомерПересчета) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ Различные
	               ...
	               |	И ПересчетТоваров.СК_НомерПересчета = &СК_НомерПересчета";
	Запрос.УстановитьПараметр("СК_НомерПересчета",СК_НомерПересчета); 
	Возврат Запрос.Выполнить().Выгрузить();
КонецФункции

///Складской учет

Функция ПолучитьОстатокПоСкладу(Номенклатура,ДатаЗапроса,Склад) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ТоварыНаСкладахОстатки.ВНаличииОстаток КАК ВНаличииОстаток
	               |ИЗ
	               |	РегистрНакопления.ТоварыНаСкладах.Остатки(&ДатаЗапроса,Номенклатура = &Номенклатура И Склад = &Склад)  КАК ТоварыНаСкладахОстатки
	               |ГДЕ
