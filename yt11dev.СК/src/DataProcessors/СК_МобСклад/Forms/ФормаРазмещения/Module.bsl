...
Процедура ШКЯчейкиИзменениеТекстаРедактирования(Элемент, Текст, СтандартнаяОбработка)
	...
	Если ЭтаФорма.Ячейка <> "" Тогда
		...
		//ПодключитьОбработчикОжидания("АктивироватьВналичии",5,Истина);
	КонецЕсли;
...
Функция ПринятьНаСервере()
	Если ЭтаФорма.Номенклатура.Ссылка.Пустая() =  Ложь И ЭтаФорма.Ячейка.Ссылка.Пустая() = Ложь И ЭтаФорма.Вналичии >=1 Тогда 
		
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
		|	Расш1_ПоступлениеСклад.Ссылка КАК Ссылка
		|ИЗ
		|	Документ.СК_ПоступлениеСклад КАК Расш1_ПоступлениеСклад
		|ГДЕ
		...
		Иначе
			ТекДок = Документы.СК_ПоступлениеСклад.СоздатьДокумент(); 
